...
from loss.Dice_3dloss import DiceLoss
...
from dataset.dataset_3d import train_fix_ds,valid_fix_ds
...
start = time()
# writer1 =SummaryWriter('./result/train-loss')
# writer2 =SummaryWriter('./result/train-iou')
# writer3 =SummaryWriter('./result/val-loss')
# writer4 =SummaryWriter('./result/val-iou')
def parse_args():
...
def train(args, train_loader, model, criterion, optimizer, scheduler,epoch):
    ...
    for i, (input, target) in tqdm(enumerate(train_loader), total=len(train_loader)):
        ...
        # index1=len(glob('/home/lc/Study/DataBase/tumor(1)/input/*-input.png'))
        # plt.imshow(input[0,0,0,:,:])
        # plt.axis('off') # 不显示坐标轴
        # plt.savefig('/home/lc/Study/DataBase/tumor(1)/input/'+str(index1)+'-input.png',dpi=200,bbox_inches='tight')
        # index2=len(glob('/home/lc/Study/DataBase/tumor(1)/gt/*-gt.png'))
        # plt.imshow(target[0,0,:,:])
        # plt.axis('off') # 不显示坐标轴
        # plt.savefig('/home/lc/Study/DataBase/tumor(1)/gt/'+str(index2)+'-gt.png',dpi=200,bbox_inches='tight')
...
def main():
    args = parse_args()
    # 定义数据加载
    if not os.path.exists('models/tumor-3D/%s' %args.name):
        os.makedirs('models/tumor-3D/%s' %args.name)
    ...
    with open('models/tumor-3D/%s/args.txt' %args.name, 'w') as f:
        for arg in vars(args):
    ...
    # 学习率衰减
    scheduler = lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=1000,
    ...
    for epoch in range(args.epochs):
        ...
        # writer1.add_scalar('train-loss',train_log['loss'],epoch)
        # writer2.add_scalar('train-iou',train_log['iou'],epoch)
        # writer3.add_scalar('val-loss',val_log['loss'],epoch)
        # writer4.add_scalar('val-iou',val_log['iou'],epoch)
        # lr_decay.step()
        ...
        log.to_csv('models/tumor-3D/%s/log.csv' %args.name, index=False)

        trigger += 1

        if val_log['iou'] > best_iou:
            torch.save(model.state_dict(), 'models/tumor-3D/%s/model.pth' %args.name)
