...
def main():
    ...
    for file_index, file in enumerate(os.listdir(val_ct_dir)):
        ...
        outputs_list = []
        for png_index in range(ct_array_list.shape[0]):

            ct_tensor = torch.FloatTensor(ct_array_list[png_index,:,:]).cuda()
            # ct_tensor = ct_tensor.unsqueeze(dim=0)
            # ct_tensor = ct_tensor.unsqueeze(dim=1)
            print(ct_tensor.shape)
            pred_seg = net(ct_tensor)
            casetime = time() - start
            print(pred_seg.shape)
            pred_seg = pred_seg.squeeze(dim=1)
            pred_seg = pred_seg.cpu().detach().numpy()

            # 将预测的结果保存为nii数据
            pred_image = sitk.GetImageFromArray(pred_seg)

            pred_image.SetDirection(ct.GetDirection())
            pred_image.SetOrigin(ct.GetOrigin())
            pred_image.SetSpacing(ct.GetSpacing())

            sitk.WriteImage(pred_image, os.path.join(liver_pred_dir, png_index+ile.replace('volume', 'pred').replace('nii', 'png')))
        ...
        print(pred_seg[1,200,200])
        # print(target.shape)
        print(seg_array[1,200,200])
        # # 先进行腐蚀
