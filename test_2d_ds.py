...
def main():
    ...
    args = joblib.load('models/LITS/%s/args.pkl' %test_args.name)
    # args = joblib.load('models/None_NestedUNet_woDS/args.pkl')

    if not os.path.exists('output/LITS/%s' %args.name):
        os.makedirs('output/LITS/%s' %args.name)
    ...
    # test_img_paths = glob('/home/lc/学习/DataBase/Unet-master/deform/y_test/*')
    ...
    model.load_state_dict(torch.load('models/LITS/%s/model.pth' %args.name))
    model.eval()
        ...
        with torch.no_grad():
            for i, (input, target) in tqdm(enumerate(test_loader), total=len(test_loader)):
                ...
                for i in range(output.shape[0]):
                    imsave('output/LITS/%s/'%args.name+os.path.basename(img_paths[i]), (output[i,:,:]))
            ...
            print('time SUM: {}'.format(sum(time_list)))
