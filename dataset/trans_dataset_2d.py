...
class Dataset(dataset):
    def __init__(self,  trans_ct_dir, seg_dir):

        self. trans_ct_list =  trans_ct_dir
        # self.num_class = 1#2
        #self.seg_list = seg_dir
        self.ct_list = list(map(lambda x: x.replace('translated', 'origin'), self. trans_ct_list))
        self.seg_list = list(map(lambda x: x.replace('image', 'mask'), self. trans_ct_list))
    ...
    def __getitem__(self, index):
        ...
        trans_ct_path = self.trans_ct_list[index]
        seg_path = self.seg_list[index]
        # print(ct_path,seg_path)
        ct = sitk.ReadImage(ct_path, sitk.sitkInt16)
        trans_ct = sitk.ReadImage(trans_ct_path, sitk.sitkInt16)
        seg = sitk.ReadImage(seg_path, sitk.sitkUInt8)

        ct_array = sitk.GetArrayFromImage(ct)
        trans_ct_array = sitk.GetArrayFromImage(trans_ct)
        seg_array = sitk.GetArrayFromImage(seg)
        # print(ct_array.shape)
        # print(seg_array.shape)
        ct_array = torch.FloatTensor(ct_array).unsqueeze(0)
        trans_ct_array = torch.FloatTensor(trans_ct_array).unsqueeze(0)
        seg_array = torch.FloatTensor(seg_array)

        return ct_array, trans_ct_array, seg_array
